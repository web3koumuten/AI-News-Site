...
try:
    ...
    print(f"Import error in ledge_ai_scraper: {e}")
    raise


class LedgeAIScraper(BaseScraper):
    """Scraper for Ledge.ai RSS feed"""
    
    def __init__(self):
        super().__init__(SOURCES['ledge_ai'])
        self.rss_url = SOURCES['ledge_ai']['rss_url']
    
    def fetch_articles(self) -> List[Dict]:
        """Fetch articles from Ledge.ai RSS feed"""
        articles = []
        ...
        try:
            ...
            if not feed.entries:
                print(f"No entries found in Ledge.ai RSS feed")
                return articles
            ...
            print(f"Error fetching Ledge.ai articles: {e}")
    ...
    def get_article_content(self, url: str) -> Optional[str]:
        """Override to handle Ledge.ai specific content extraction"""
        soup = self.fetch_page(url)
        if not soup:
            return None
        
        # Ledge.ai specific content selectors
        content_selectors = [
            '.entry-content',
            '.article-content',
            '.post-content',
        ...
        for selector in content_selectors:
            ...
            if content_elem:
                ...
                for elem in content_elem.select('script, style, .ad, .share, .related, .author-box, .newsletter'):
                    elem.decompose()
    ...
    def _extract_image_url(self, soup) -> Optional[str]:
        ...
        # Ledge.ai specific image selectors
        selectors = [
