...
try:
    ...
    print(f"Import error in ainow_scraper: {e}")
    raise


class AINOWScraper(BaseScraper):
    """Scraper for AINOW RSS feed"""
    
    def __init__(self):
        super().__init__(SOURCES['ainow'])
        self.rss_url = SOURCES['ainow']['rss_url']
    
    def fetch_articles(self) -> List[Dict]:
        """Fetch articles from AINOW RSS feed"""
        articles = []
        ...
        try:
            ...
            if not feed.entries:
                print(f"No entries found in AINOW RSS feed")
                return articles
            ...
            print(f"Error fetching AINOW articles: {e}")
    ...
    def get_article_content(self, url: str) -> Optional[str]:
        """Override to handle AINOW specific content extraction"""
        soup = self.fetch_page(url)
        if not soup:
            return None
        
        # AINOW specific content selectors
        content_selectors = [
            '.entry-content',
            '.post-content',
            '.article-content',
        ...
        for selector in content_selectors:
            ...
            if content_elem:
                ...
                for elem in content_elem.select('script, style, .related-articles, .ads, .social-share, .navigation'):
                    elem.decompose()
    ...
    def _extract_image_url(self, soup) -> Optional[str]:
        ...
        # AINOW specific image selectors
        selectors = [
